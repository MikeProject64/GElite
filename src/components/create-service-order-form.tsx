...
import { collection, addDoc, query, where, onSnapshot, Timestamp, orderBy, getDocs, doc, getDoc, updateDoc, setDoc } from 'firebase/firestore';
...
import { Loader2, CalendarIcon, ChevronsUpDown, Check, PlusCircle } from 'lucide-react';
import { ScrollArea } from '@/components/ui/scroll-area';
import type { Customer, Collaborator, ServiceOrder } from '@/types';
...
const serviceOrderSchema = z.object({
  ...
  collaboratorId: z.string().min(1, "Selecione um responsável."),
  ...
  warrantyDays: z.preprocess(
    ...
    z.number().int("Deve ser um número inteiro.").positive("Deve ser um número positivo.").optional()
  ),
...
export function CreateServiceOrderForm({ onFormSubmit, isModal = false }: CreateServiceOrderFormProps) {
  const { user } = useAuth();
  ...
  }, [searchParams, user, form, toast, router, settings.serviceOrderCustomFields, isModal]);
  ...
  }, [isNewClientDialogOpen, isNewServiceTypeDialogOpen, newServiceTypeForm, isNewCollaboratorDialogOpen, newCollaboratorForm]);
  ...
  const onServiceOrderSubmit = async (data: ServiceOrderValues) => {
    ...
    try {
      ...
      if (isVersioning && baseOrder) {
        ...
        payload.attachments = baseOrder.attachments;
        payload.activityLog = [
            ...(baseOrder.activityLog || []),
            {
                timestamp: Timestamp.now(),
                userEmail: user?.email || 'Sistema',
      ...
      await updateDoc(docRef, { originalServiceOrderId: payload.originalServiceOrderId || docRef.id });
  ...
  if (isInitializing && !isModal) {
    return <div className="flex justify-center items-center h-64"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>;
  }
  
  const clientIdFromUrl = !isModal ? searchParams.get('clientId') : null;
  const formContent = (
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onServiceOrderSubmit)} className="space-y-6">
              ...
              <Popover open={isCustomerDropdownOpen} onOpenChange={setIsCustomerDropdownOpen}>
                <PopoverTrigger asChild>
                  <Button type="button" variant="outline" role="combobox" className={cn("w-full justify-between", !field.value && "text-muted-foreground")} disabled={isVersioning || !!clientIdFromUrl}>
                    <span className='truncate'>{field.value ? customers.find(c => c.id === field.value)?.name : "Selecione um cliente"}</span>
                    <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-[--radix-popover-trigger-width] p-0" align="start">
                  <Command><CommandInput placeholder="Buscar cliente..." onValueChange={setCustomerSearchTerm} /><CommandList><CommandEmpty>Nenhum cliente encontrado.</CommandEmpty><CommandGroup><ScrollArea className="h-48">{filteredCustomers.map((customer) => (<CommandItem key={customer.id} value={customer.id} onSelect={(currentValue) => {form.setValue('clientId', currentValue === field.value ? '' : currentValue, { shouldValidate: true }); setIsCustomerDropdownOpen(false);}}><Check className={cn("mr-2 h-4 w-4", field.value === customer.id ? "opacity-100" : "opacity-0")} /><div><p>{customer.name}</p><p className="text-xs text-muted-foreground">{customer.phone}</p></div></CommandItem>))}</ScrollArea></CommandGroup></CommandList></Command>
                </PopoverContent>
              </Popover>
                    <Button type="button" variant="secondary" size="icon" onClick={() => setIsNewClientDialogOpen(true)} disabled={isVersioning || !!clientIdFromUrl}><PlusCircle className="h-4 w-4" /><span className="sr-only">Adicionar Novo Cliente</span></Button>
                </div><FormMessage />
            </FormItem>
          )}/>
          <FormField control={form.control} name="serviceType" render={({ field }) => (<FormItem><FormLabel>Serviço *</FormLabel><FormControl><Input placeholder="Ex: Manutenção de Ar Condicionado" {...field} /></FormControl><FormMessage /></FormItem>)}/>
          <FormField control={form.control} name="serviceCategory" render={({ field }) => (
            <FormItem>
                <FormLabel>Tipo de Serviço</FormLabel>
                 <div className="flex w-full items-center gap-2">
                <Select onValueChange={field.onChange} value={field.value} defaultValue={field.value}><FormControl><SelectTrigger>{field.value ? (<div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full" style={{ backgroundColor: `hsl(${settings.serviceTypes?.find(s => s.name === field.value)?.color || statusColors[0].value})` }}></div><span>{field.value}</span></div>) : (<SelectValue placeholder="Selecione o tipo (opcional)" />)}</SelectTrigger></FormControl>
                <SelectContent>{settings.serviceTypes && settings.serviceTypes.length > 0 ? (settings.serviceTypes.map((cat: any) => (<SelectItem key={cat.id} value={cat.name}><div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full" style={{ backgroundColor: `hsl(${cat.color || statusColors[0].value})` }}></div><span>{cat.name}</span></div></SelectItem>))) : (<div className="p-2 text-muted-foreground">Nenhum tipo cadastrado</div>)}</SelectContent>
                </Select>
                    <Button type="button" variant="secondary" size="icon" onClick={() => setIsNewServiceTypeDialogOpen(true)}><PlusCircle className="h-4 w-4" /><span className="sr-only">Adicionar Novo Tipo</span></Button>
                </div><FormMessage />
            </FormItem>
            )}/>
          <FormField control={form.control} name="problemDescription" render={({ field }) => (<FormItem><FormLabel>Descrição do Problema *</FormLabel><FormControl><Textarea placeholder="Detalhe o problema relatado pelo cliente..." {...field} /></FormControl><FormMessage /></FormItem>)}/>
           <FormField control={form.control} name="collaboratorId" render={({ field }) => (
              <FormItem>
                ...
                <div className="flex w-full items-center gap-2">
                <Select onValueChange={field.onChange} value={field.value} defaultValue={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Selecione um responsável" /></SelectTrigger></FormControl>
                  <SelectContent>{collaborators.map((c) => (<SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>))}</SelectContent>
                </Select>
                  <Button type="button" variant="secondary" size="icon" onClick={() => setIsNewCollaboratorDialogOpen(true)}><PlusCircle className="h-4 w-4" /><span className="sr-only">Adicionar Novo Responsável</span></Button>
                </div><FormMessage />
              </FormItem>
            )}/>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
               <FormField control={form.control} name="totalValue" render={({ field }) => (
                <FormItem className='md:col-span-2'>
                  ...
                  <FormControl><div className="relative"><span className="absolute inset-y-0 left-0 flex items-center pl-3 text-muted-foreground">R$</span><Input type="number" step="0.01" placeholder="250,00" className="pl-9" {...field} value={field.value || ''} /></div></FormControl><FormMessage />
                </FormItem>
              )}/>
               <FormField control={form.control} name="warrantyDays" render={({ field }) => (
                <FormItem>
                  <FormLabel>Garantia (dias)</FormLabel>
                  <FormControl><Input type="number" step="1" placeholder="Ex: 90" {...field} value={field.value || ''} /></FormControl><FormMessage />
                </FormItem>
              )}/>
            </div>
          <FormField control={form.control} name="dueDate" render={({ field }) => (
            <FormItem className="flex flex-col"><FormLabel>Data de Vencimento *</FormLabel>
              <Popover><PopoverTrigger asChild><FormControl><Button variant={"outline"} className={cn("w-full pl-3 text-left font-normal", !field.value && "text-muted-foreground")}><span className='flex w-full items-center justify-between'>{field.value ? format(field.value, "PPP", { locale: ptBR }) : <span>Selecione a data</span>}<CalendarIcon className="h-4 w-4 opacity-50" /></span></Button></FormControl></PopoverTrigger><PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={field.value ?? undefined} onSelect={field.onChange} initialFocus /></PopoverContent></Popover><FormMessage />
            </FormItem>
          )}/>
          <div className="grid grid-cols-2 gap-4">
            <FormField control={form.control} name="status" render={({ field }) => (
                ...
                <Select onValueChange={field.onChange} value={field.value} defaultValue={field.value}><FormControl><SelectTrigger>{field.value ? (<div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full" style={{ backgroundColor: `hsl(${settings.serviceStatuses?.find(s => s.name === field.value)?.color})` }}></div><span>{field.value}</span></div>) : (<SelectValue placeholder="Selecione o status inicial" />)}</SelectTrigger></FormControl>
                    <SelectContent>{settings.serviceStatuses?.map(status => (<SelectItem key={status.id} value={status.name}><div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full" style={{ backgroundColor: `hsl(${status.color})` }}></div><span>{status.name}</span></div></SelectItem>))}</SelectContent>
                </Select><FormMessage />
                </FormItem>
            )}/>
            <FormField control={form.control} name="priority" render={({ field }) => (
                ...
                <Select onValueChange={field.onChange} value={field.value} defaultValue={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Defina a prioridade" /></SelectTrigger></FormControl>
                                ...
                                <FormItem>
                                    ...
                                    <FormControl>
                                        {customField.type === 'date' ? (
                                            <Popover>
                                                <PopoverTrigger asChild><Button variant={"outline"} className={cn("w-full justify-start text-left font-normal", !field.value && "text-muted-foreground")}><CalendarIcon className="mr-2 h-4 w-4" />{field.value ? format(new Date(field.value), "PPP", { locale: ptBR }) : <span>Selecione a data</span>}</Button></PopoverTrigger>
                                                <PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={field.value ? new Date(field.value) : undefined} onSelect={field.onChange} initialFocus /></PopoverContent>
                                            </Popover>
                                        ) : (
          ...
          <div className="flex justify-end gap-2 pt-4">
              <Button type="button" variant="ghost" onClick={() => isModal ? onFormSubmit?.() : router.back()}>{isModal ? "Cancelar" : "Voltar"}</Button>
              <Button type="submit" disabled={form.formState.isSubmitting}>{form.formState.isSubmitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Salvar Ordem de Serviço</Button>
          </div>
        </form>
      </Form>
  );
  ...
  return (
    <>
      ...
      <Dialog open={isNewClientDialogOpen} onOpenChange={setIsNewClientDialogOpen}>
        <DialogContent className="sm:max-w-2xl"><DialogHeader><DialogTitle>Cadastrar Novo Cliente</DialogTitle><DialogDescription>Preencha os detalhes para um cadastro rápido.</DialogDescription></DialogHeader><div className="max-h-[70vh] overflow-y-auto p-1"><CustomerForm onSubmit={onNewClientSubmit} onCancel={() => setIsNewClientDialogOpen(false)} /></div></DialogContent>
      </Dialog>
      <Dialog open={isNewServiceTypeDialogOpen} onOpenChange={setIsNewServiceTypeDialogOpen}>
        <DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>Adicionar Novo Tipo de Serviço</DialogTitle></DialogHeader>
            <Form {...newServiceTypeForm}>
                <form onSubmit={newServiceTypeForm.handleSubmit(onNewServiceTypeSubmit)} className="space-y-4">
                    <FormField control={newServiceTypeForm.control} name="name" render={({ field }) => (<FormItem><FormLabel>Nome do Tipo</FormLabel><FormControl><Input placeholder="Ex: Limpeza, Instalação..." {...field} /></FormControl><FormMessage /></FormItem>)}/>
                    <FormField control={newServiceTypeForm.control} name="color" render={({ field }) => (<FormItem><FormLabel>Cor</FormLabel><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger className="w-[80px]"><SelectValue><div className="w-4 h-4 rounded-full" style={{backgroundColor: `hsl(${field.value})`}}></div></SelectValue></SelectTrigger></FormControl><SelectContent>{statusColors.map(color => (<SelectItem key={color.value} value={color.value}><div className='flex items-center gap-2'><div className="w-4 h-4 rounded-full" style={{backgroundColor: `hsl(${color.value})`}}></div>{color.name}</div></SelectItem>))}</SelectContent></Select><FormMessage /></FormItem>)}/>
                    <DialogFooter><Button type="button" variant="ghost" onClick={() => setIsNewServiceTypeDialogOpen(false)}>Cancelar</Button><Button type="submit" disabled={newServiceTypeForm.formState.isSubmitting}>{newServiceTypeForm.formState.isSubmitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Adicionar</Button></DialogFooter>
                </form>
            </Form>
        </DialogContent>
      </Dialog>
      <Dialog open={isNewCollaboratorDialogOpen} onOpenChange={setIsNewCollaboratorDialogOpen}>
        <DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>Adicionar Novo Responsável</DialogTitle></DialogHeader>
            <Form {...newCollaboratorForm}>
                <form onSubmit={newCollaboratorForm.handleSubmit(onNewCollaboratorSubmit)} className="space-y-4">
                    <FormField control={newCollaboratorForm.control} name="name" render={({ field }) => (<FormItem><FormLabel>Nome</FormLabel><FormControl><Input placeholder="Ex: João Silva, Setor de Limpeza..." {...field} /></FormControl><FormMessage /></FormItem>)}/>
                    <FormField control={newCollaboratorForm.control} name="type" render={({ field }) => (<FormItem><FormLabel>Tipo</FormLabel><Select onValueChange={field.onChange} defaultValue={field.value}><FormControl><SelectTrigger><SelectValue /></SelectTrigger></FormControl><SelectContent><SelectItem value="collaborator">Colaborador</SelectItem><SelectItem value="sector">Setor</SelectItem></SelectContent></Select><FormMessage /></FormItem>)}/>
                    <DialogFooter><Button type="button" variant="ghost" onClick={() => setIsNewCollaboratorDialogOpen(false)}>Cancelar</Button><Button type="submit" disabled={newCollaboratorForm.formState.isSubmitting}>{newCollaboratorForm.formState.isSubmitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Adicionar</Button></DialogFooter>
              </form>
            </Form>
        </DialogContent>
      </Dialog>
